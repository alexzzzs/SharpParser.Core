...
/// Module for parallel processing functionality in the parsing engine
module ParallelProcessing =
    ...
    let private processPosition (config: ParserConfig) (context: ParserContext) (line: string) (pos: int) : ParserContext * int =
    ...
    let private processLine (config: ParserConfig) (context: ParserContext) (line: string) : ParserContext =
    ...
    let private processInput (config: ParserConfig) (context: ParserContext) (lines: string seq) : ParserContext =
    ...
    let defaultBoundaryPatterns = [
        ...
        {
            DeclarationPattern = @"^\s*(?:const|let|var)\s+(\w+)\s*=\s*(?:\([^)]*\)\s*)?=>\s*\{"  // Fixed regex
            NameGroupIndex = 1
            ...
            if functions.Length < config.ParallelConfig.MinFunctionsForParallelism then
                ...
                let functionResults =
                    ...
                    |> Array.Parallel.map (fun func ->
                        ...
                        let processedContext = processInput isolatedConfig funcContext funcLines
